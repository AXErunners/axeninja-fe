...
$(document).ready(function(){
    ...
    $('#budgetsdetailtable').on('xhr.dt', function ( e, settings, json ) {
        ...
        $('#globalestablishedbudgetamount').text(addCommas(totalamount)+' '+axeninjacoin[axeninjatestnet]);
...
//        $('#globalnextsuperblockunallocated').text(addCommas(Math.round((json.data.stats.nextsuperblock.estimatedbudgetamount-totalamount)*100)/100)+' '+axeninjacoin[axeninjatestnet]);
    ...
    tableBudgets = $('#budgetsdetailtable').dataTable( {
        ajax: { url: "/api/budgets?testnet="+axeninjatestnet,
        ...
        columns: [
            ...
            { data: null, render: function ( data, type, row ) {
                var outtxt = data.ID;
                if (type != 'sort') {
                    outtxt = '<a href="'+axeninjabudgetdetail[axeninjatestnet].replace('%%b%%',data.ID)+'">'+data.ID+'</a>';
                }
                return outtxt;
            } },
            { data: null, render: function ( data, type, row ) {
                ...
                if (type == 'sort') {
                    ...
                    if (axeninjatxexplorer[axeninjatestnet].length > 0) {
                        ...
                        for ( var i=0, ien=axeninjatxexplorer[axeninjatestnet].length ; i<ien ; i++ ) {
                            if (ix == 0) {
                                outtxt += '<a href="'+axeninjatxexplorer[axeninjatestnet][0][0].replace('%%a%%',data.FeeHash)+'">'+data.FeeHash.substring(0,7)+'</a>';
                            }
                            else {
                                outtxt += '<a href="'+axeninjatxexplorer[axeninjatestnet][i][0].replace('%%a%%',data.FeeHash)+'">['+ix+']</a>';
                            }
                            ix++;
                        }
                    }
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == 'sort') {
                    ...
                    return addCommas(data.TotalPayment+' '+axeninjacoin[axeninjatestnet]);
                }
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == 'sort') {
                    ...
                    return addCommas(data.MonthlyPayment+' '+axeninjacoin[axeninjatestnet]);
                }
                ...
                if (type == 'sort') {
                    ...
                    if (axeninjaaddressexplorer[axeninjatestnet].length > 0) {
                        ...
                        for ( var i=0, ien=axeninjaaddressexplorer[axeninjatestnet].length ; i<ien ; i++ ) {
                            if (ix == 0) {
                                outtxt += '<a href="'+axeninjaaddressexplorer[axeninjatestnet][0][0].replace('%%a%%',data.PaymentAddress)+'">'+data.PaymentAddress+'</a>';
                            }
                            else {
                                outtxt += '<a href="'+axeninjaaddressexplorer[axeninjatestnet][i][0].replace('%%a%%',data.PaymentAddress)+'">['+ix+']</a>';
                            }
                            ix++;
                        }
                    }
    ...
    $('#budgetsprojectiondetailtable').on('xhr.dt', function ( e, settings, json ) {
        ...
        $('#globalallotedbudgetamount').text(addCommas(totalamount)+' '+axeninjacoin[axeninjatestnet]);
        $('#globalnextsuperblockunallocated').text(addCommas(Math.round((json.data.stats.nextsuperblock.estimatedbudgetamount-totalamount)*100)/100)+' '+axeninjacoin[axeninjatestnet]);
    ...
    tableBudgetsProjection = $('#budgetsprojectiondetailtable').dataTable( {
        ajax: { url: "/api/budgetsprojection?testnet="+axeninjatestnet,
        ...
        columns: [
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == 'sort') {
                    ...
                    return addCommas(data.Alloted+' '+axeninjacoin[axeninjatestnet]);
                }
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == 'sort') {
                    ...
                    return addCommas(data.TotalBudgetAlloted+' '+axeninjacoin[axeninjatestnet]);
                }
    ...
    tableSuperBlocks = $('#superblockstable').dataTable( {
        ajax: { url: "/api/blocks?testnet="+axeninjatestnet+"&onlysuperblocks=1",
            dataSrc: 'data.blocks' },
        paging: false,
        order: [[ 0, "desc" ]],
        columns: [
...
//                return deltaTimeStampHR(currenttimestamp(),data.BlockTime);
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == "sort") {
                    ...
                    return '<a href="' + axeninjabudgetdetail[axeninjatestnet].replace('%%b%%',encodeURIComponent(data.SuperBlockBudgetName)) + '">' + data.SuperBlockBudgetName + '</a>';
                }
              }
            },
            { data: null, render: function ( data, type, row ) {
    ...
    tableSuperBlocksExpected = $('#superblocksexpectedtable').dataTable( {
        ajax: { url: "/api/budgetsexpected?testnet="+axeninjatestnet,
        ...
        columns: [
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == "sort") {
                    ...
                    return '<a href="' + axeninjabudgetdetail[axeninjatestnet].replace('%%b%%',encodeURIComponent(data.BlockProposal)) + '">' + data.BlockProposal + '</a>';
                }
            }
            },
            { data: null, render: function ( data, type, row ) {
                if (type == "sort") {
                    ...
                    return addCommas(data.MonthlyPayment.toFixed(3))+" "+axeninjacoin[axeninjatestnet];
                }
            }
            }
        ],
