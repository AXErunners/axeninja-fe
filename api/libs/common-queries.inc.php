...
function dmn_masternodes_get($mysqli, $testnet = 0, $protocol = 0, $mnpubkeys = array(), $mnips = array(), $withlastpaid = false) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("axeninja_masternodes_get_%d_%d_%d_%d_%d_%s",$testnet,$protocol,count($mnpubkeys),count($mnips),$lastpaidnum,$cacheserial);
...
function dmn_masternodes2_get($mysqli, $testnet = 0, $protocol = 0, $mnpubkeys = array(), $mnips = array(), $mnvins = array()) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("axeninja_masternodes2_get_%d_%d_%d_%d_%d_%s",$testnet,$protocol,count($mnpubkeys),count($mnips),count($mnvins),$cacheserial);
...
function dmn_protx_get($mysqli, $testnet = 0, $protxhashes= array(), $protxips= array(), $collaterals= array(), &$cachevalid = false, $withstatepernode = false) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("axeninja_protx_get_%d_%d_%d_%d_%s",$testnet,count($protxhashes),count($protxips),count($collaterals),$cacheserial);
    $cachefnamupdate = $cachefnam.".update";
    $cachevalid = (is_readable($cachefnam) && (((filemtime($cachefnam)+300)>=time()) || file_exists($cachefnamupdate)));
    if (DMN_USE_CACHE && $cachevalid) {
        $nodes = unserialize(file_get_contents($cachefnam));
...
function dmn_masternodes_votes_get($mysqli, $mnips = array(), $testnet) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("axeninja_masternodes_votes_get_%d_%d_%s",$testnet,count($mnips),$cacheserial);
    $cachevalid = (is_readable($cachefnam) && ((filemtime($cachefnam)+300)>=time()));
    if (DMN_USE_CACHE && $cachevalid) {
        $nodes = unserialize(file_get_contents($cachefnam));
    }
...
//    $cachefnam = CACHEFOLDER.sprintf("axeninja_masternodes_portcheck_get_%d_%d_%s",$testnet,count($mnkeys),$cacheserial);
    $cachefnam = CACHEFOLDER.sprintf("axeninja_masternodes_portcheck_get_%d",$testnet);
    $cachefnamupdate = $cachefnam.".update";
    $cachevalid = (is_readable($cachefnam) && (((filemtime($cachefnam)+300)>=time()) || file_exists($cachefnamupdate)));
    if (DMN_USE_CACHE && $cachevalid) {
...
function dmn_masternodes_donation_get($mysqli, $mnkeys, $testnet = 0) {
    ...
    $cacheserial = sha1(serialize($mnkeys));
    $cachefnam = CACHEFOLDER.sprintf("axeninja_masternodes_donation_get_%d_%d_%s",$testnet,count($mnkeys),$cacheserial);
    $cachevalid = (is_readable($cachefnam) && ((filemtime($cachefnam)+300)>=time()));
    if (DMN_USE_CACHE && $cachevalid) {
...
function dmn_masternodes_donations_get($mysqli, $testnet = 0) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("axeninja_masternodes_donations_get_%d",$testnet);
    $cachevalid = (is_readable($cachefnam) && ((filemtime($cachefnam)+300)>=time()));
    if (DMN_USE_CACHE && $cachevalid) {
        $donation = unserialize(file_get_contents($cachefnam));
    }
...
function dmn_masternodes_balance_get($mysqli, $mnkeys, $testnet = 0) {
    ...
    $cacheserial = sha1(serialize($mnkeys));
    $cachefnam = CACHEFOLDER.sprintf("axeninja_masternodes_balance_get_%d_%d_%s",$testnet,count($mnkeys),$cacheserial);
    $cachevalid = (is_readable($cachefnam) && ((filemtime($cachefnam)+300)>=time()));
    if (DMN_USE_CACHE && $cachevalid) {
...
function dmn_masternodes_exstatus_get($mysqli, $mnkeys, $testnet = 0) {
    ...
    $cacheserial = sha1(serialize($mnkeys));
    $cachefnam = CACHEFOLDER.sprintf("axeninja_masternodes_exstatus_get_%d_%d_%s",$testnet,count($mnkeys),$cacheserial);
...
function dmn_masternodes_count($mysqli, $testnet, &$totalmncount, &$uniquemnips) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("axeninja_protocols_%d",$testnet);
    $cachevalid = (is_readable($cachefnam) && ((filemtime($cachefnam)+300)>=time()));
    $protocols = array();
    if (DMN_USE_CACHE && $cachevalid) {
