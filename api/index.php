...
$app->get('/api/blocks', function() use ($app,&$mysqli) {
  ...
  if (count($errmsg) > 0) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("axeninja_blocks_%d_%d_%s_%d_%d_%d_%s",$testnet,$cachenodetail,$cacheinterval,count($mnpubkeys),$onlysuperblocks,count($budgetids),$cacheserial);
      ...
      if ($result = $mysqli->query($sql)) {
        ...
        $data = array('blocks' => $blocks,
            ...
            'api' => array(
                ...
                'bev' => 'bk='.AXENINJA_BEV.".".$apiversion
            )
                                                                         );
        //Change the HTTP status
        $response->setStatusCode(200, "OK");
...
$app->get('/api/blocks/superblocks', function() use ($app,&$mysqli) {
    ...
    if (count($errmsg) > 0) {
        ...
        $cachefnam = CACHEFOLDER.sprintf("axeninja_blocks_superblockspayments_%d_%d_%s",$testnet,count($proposalshash),$cacheserial);
        $cachefnamupdate = $cachefnam.".update";
        $cachevalid = (is_readable($cachefnam) && (((filemtime($cachefnam)+$cachetime)>=time()) || file_exists($cachefnamupdate)));
        if (DMN_USE_CACHE && $cachevalid) {
            $data = unserialize(file_get_contents($cachefnam));
            ...
            if ($result = $mysqli->query($sql)) {
                ...
                $data = array('superblocks' => $superblocks,
                    ...
                    'api' => array(
                        'version' => $apiversion,
                        'compat' => $apiversioncompat,
                        'bev' => 'sb='.AXENINJA_BEV.".".$apiversion
                    )
                );
                //Change the HTTP status
                $response->setStatusCode(200, "OK");
...
$app->get('/api/budgets', function() use ($app,&$mysqli) {
  ...
  if (count($errmsg) > 0) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("axeninja_budgets_%d_%d_%d_%d_%s",$testnet,$onlyvalid,count($budgetids),count($budgethashes),$cacheserial);
    $cachefnamupdate = $cachefnam.".update";
      ...
      if ($result = $mysqli->query($sql)) {
        ...
        $data = array('budgets' => $budgets,
            ...
            'api' => array(
                'version' => $apiversion,
                'compat' => $apiversioncompat,
                'bev' => 'bu='.AXENINJA_BEV.".".$apiversion
            )
                     );
...
$app->get('/api/budgetsexpected', function() use ($app,&$mysqli) {
  ...
  if (count($errmsg) > 0) {
    ...
    $cachefnam = CACHEFOLDER . sprintf("axeninja_budgets_final_%d", $testnet);
    $cachefnamupdate = $cachefnam . ".update";
        ...
        if ($result = $mysqli->query($sql)) {
          ...
          $data = array(
              ...
              'api' => array(
                  'version' => $apiversion,
                  'compat' => $apiversioncompat,
                  'bev' => 'be=' . AXENINJA_BEV . "." . $apiversion
              )
          );
...
$app->get('/api/budgets/votes', function() use ($app,&$mysqli) {
  ...
  if (count($errmsg) > 0) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("axeninja_budgets_votes_%d_%d_%s",$testnet,$onlyvalid,$cacheserial);
    $cachefnamupdate = $cachefnam.".update";
    $cachetime = filemtime($cachefnam);
    $cachevalid = (is_readable($cachefnam) && ((($cachetime+120)>=time()) || file_exists($cachefnamupdate)));
    if (DMN_USE_CACHE && $cachevalid) {
      ...
      if ($result = $mysqli->query($sql)) {
        ...
        $data = array('budgetsvotes' => $budgetsvotes,
            ...
            'api' => array(
                'version' => $apiversion,
                'compat' => $apiversioncompat,
                'bev' => 'bv='.AXENINJA_BEV.".".$apiversion
            )
        );
...
$app->get('/api/budgetsprojection', function() use ($app,&$mysqli) {
  ...
  if (count($errmsg) > 0) {
    ...
    $cacheserial = sha1(serialize($budgetids).serialize($budgethashes));
    $cachefnam = CACHEFOLDER.sprintf("axeninja_budgetsprojection_%d_%d_%d_%d_%s",$testnet,$onlyvalid,count($budgetids),count($budgethashes),$cacheserial);
    $cachefnamupdate = $cachefnam.".update";
    $cachevalid = (is_readable($cachefnam) && (((filemtime($cachefnam)+120)>=time()) || file_exists($cachefnamupdate)));
    if (DMN_USE_CACHE && $cachevalid) {
      $data = unserialize(file_get_contents($cachefnam));
      ...
      if ($result = $mysqli->query($sql)) {
        ...
        $data = array('budgetsprojection' => $budgets,
            ...
            'api' => array(
                'version' => $apiversion,
                'compat' => $apiversioncompat,
                'bev' => 'bp='.AXENINJA_BEV.".".$apiversion
            )
        );
...
$app->get('/api/governanceproposals', function() use ($app,&$mysqli) {
    ...
    if (count($errmsg) > 0) {
        ...
        $cachefnam = CACHEFOLDER.sprintf("axeninja_governanceproposals_%d_%d_%d_%d_%s",$testnet,$onlyvalid,count($proposalsnames),count($proposalshashes),$cacheserial);
        $cachefnamupdate = $cachefnam.".update";
        $cachevalid = (is_readable($cachefnam) && (((filemtime($cachefnam)+120)>=time()) || file_exists($cachefnamupdate)));
        if (DMN_USE_CACHE && $cachevalid) {
            $data = unserialize(file_get_contents($cachefnam));
            ...
            if ($result = $mysqli->query($sql)) {
                ...
                $data = array('governanceproposals' => $proposals,
                    ...
                    'api' => array(
                        'version' => $apiversion,
                        'compat' => $apiversioncompat,
                        'bev' => 'gp='.AXENINJA_BEV.".".$apiversion
                    )
                );
...
$app->get('/api/governanceproposals/votelimit', function() use ($app,&$mysqli) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("axeninja_governanceproposals_votelimit_%d",$testnet);
    $cachefnamupdate = $cachefnam.".update";
    $cachevalid = (is_readable($cachefnam) && (((filemtime($cachefnam)+120)>=time()) || file_exists($cachefnamupdate)));
    if (DMN_USE_CACHE && $cachevalid) {
        $data = unserialize(file_get_contents($cachefnam));
        ...
        $data = array('votelimit' => array(
            ...
            'api' => array(
                'version' => $apiversion,
                'compat' => $apiversioncompat,
                'bev' => 'gpvl=' . AXENINJA_BEV . "." . $apiversion
            )
        );
...
$app->get('/api/governanceproposals/votes', function() use ($app,&$mysqli) {
    ...
    if (count($errmsg) > 0) {
        ...
        $cacheserial = sha1(serialize($budgetid));
        $cachefnam = CACHEFOLDER.sprintf("axeninja_governanceproposals_votes_%d_%s",$testnet,$cacheserial);
        $cachefnamupdate = $cachefnam.".update";
        $cachetime = filemtime($cachefnam);
        $cachevalid = (is_readable($cachefnam) && ((($cachetime+120)>=time()) || file_exists($cachefnamupdate)));
        if (DMN_USE_CACHE && $cachevalid) {
            ...
            if ($result = $mysqli->query($sql)) {
                ...
                $data = array('governanceproposalvotes' => $governanceproposalvotes,
                    ...
                    'api' => array(
                        'version' => $apiversion,
                        'compat' => $apiversioncompat,
                        'bev' => 'gpv='.AXENINJA_BEV.".".$apiversion
                    )
                );
...
$app->get('/api/governancetriggers', function() use ($app,&$mysqli) {
    ...
    if (count($errmsg) > 0) {
        ...
        $cachefnam = CACHEFOLDER.sprintf("axeninja_governancetriggers_%d_%d_%d_%d",$testnet,$onlyvalid,$onlyfuture,$afterblockheight);
        $cachefnamupdate = $cachefnam.".update";
        $cachevalid = (is_readable($cachefnam) && (((filemtime($cachefnam)+120)>=time()) || file_exists($cachefnamupdate)));
        if (DMN_USE_CACHE && $cachevalid) {
            $data = unserialize(file_get_contents($cachefnam));
            ...
            if ($result = $mysqli->query($sql)) {
                ...
                $data = array('governancetriggers' => $triggers,
                    ...
                    'api' => array(
                        'version' => $apiversion,
                        'compat' => $apiversioncompat,
                        'bev' => 'gt='.AXENINJA_BEV.".".$apiversion
                    )
                );
...
//   prev12=0|1 (optional, respond as pre v0.12 Axe Ninja API, obsolete)
...
$app->get('/api/masternodes', function() use ($app,&$mysqli) {
  ...
  if ($protocol == -1) {
    $cachefnam = CACHEFOLDER.sprintf("axeninja_maxprotocol_%d",$testnet);
...
$app->get('/api/tablevars', function() use ($app,&$mysqli) {
  ...
  if (count($errmsg) > 0) {
    ...
    $cachefnam = CACHEFOLDER."axeninja_tablevars";
    $cachefnamupdate = $cachefnam.".update";
      ...
      if ($result = $mysqli->query($sql)) {
        ...
        $data = array("tablevars" => $stats,
            ...
            'api' => array(
                'version' => $apiversion,
                'compat' => $apiversioncompat,
                'bev' => 'tv='.AXENINJA_BEV.".".$apiversion
            )
        );
